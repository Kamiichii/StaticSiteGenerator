...
def generate_page(basepath,from_path,template_path,dest_path):
    ...
    new_template = template_contents.replace("{{ Title }}",title).replace("{{ Content }}",html).replace('href="/',f'href="{basepath}').replace('src="/',f'src="{basepath}')
...
def generate_pages_recursive(basepath,dir_path_content,template_path,dest_dir_path):
    ...
    for name in os.listdir(dir_path_content):
        ...
        if os.path.isdir(dir_path):
            generate_pages_recursive(basepath,dir_path,template_path,dest_path)
        elif re.match(r"(.*?).md$",name):
            dest_path = pathlib.Path(dest_path).with_suffix(".html")
            generate_page(basepath,dir_path, template_path, dest_path)
